...
- 前日夜の投稿が上流データ欠損などで不成立だった場合に備え、12:30 JSTの当日復旧cronを持ちます。対象日は当日で、`--recovery-only`により対象日の日次動画が既に終端状態（scheduled / private_review / published）なら生成せず即終了します。未投稿の種別が残っている場合だけ生成・投稿し、収録可能レースが0件の日は警告扱いで正常終了します。予約公開時刻は対象日前日19:00 JSTが基準のため、当日復旧では`YOUTUBE_MAX_PUBLISH_SHIFT_MINUTES`（既定240分）では必ず遅延ガードに掛かります。復旧起動時だけ`YOUTUBE_RECOVERY_MAX_PUBLISH_SHIFT_MINUTES`（既定2880分）を使い、45分の最低猶予を保ったまま起動直後の枠へ予約します。
