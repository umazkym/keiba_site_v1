...
含水率が一定の数値を超えれば自動的に稍重になる、という区分ではない。JRAでは馬場担当者が各所を踏査し、全体の状態から判断する。含水率はその参考となる数値で、芝では競馬場ごとの路盤の違いもある。[JRA「馬場状態区分の決め方」](https://www.jra.go.jp/keiba/baba/kaisetsu/)で確認できる。
