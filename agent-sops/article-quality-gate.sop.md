...
- You MUST order grade-race write orders so that the first pre-race article for a race without any published stage outranks every staged update and result review, because searches happen before the race; when post-race reviews had top priority, the three daily news slots were consumed by reviews and updates, every local grade article on 2026-09-22〜25 first appeared as a next-day review, and article search clicks fell to about one tenth of July.
- You MUST keep a result review for a race that never had a pre-race article below ordinary news candidates, and you MUST check the planner's grade-race ordering against the `D-16未公開警告` issues before changing priorities.
