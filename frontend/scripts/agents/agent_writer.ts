...
- PC検索ではtitleの前半だけで比較されるため、「データ分析」「徹底解説」だけで終わらせず、買い時・疑う条件・当日の確認順序のいずれかを明示する【禁止事項】
...
- 【関連記事リンクの禁止】関連記事はシステム側が自動で表示するため、記事本文には「関連記事」セクションや「[関連記事：ダミー]」を含めない。存在確認できないスラッグへのリンクを生成すると404エラーになる。��〇〇賞を勝った」など）、対戦成績、個別の脚質（逃げ・差しなど）など、入力である WriteOrder に明記されていない馬の個別事実は絶対に捏造・記述してはならない。WriteOrder に個別馬情報が含まれていない場合は、馬個人のことには一切言及せず、競馬場やコース等のマクロデータ（全体傾向・統計情報）の解説のみに徹底すること。
...
- ディスクリプション：120〜160文字。「この記事を読むと何ができるか」を明示。タイトル同様、過去の集計年と「最新」の混同表記は【絶対に禁止】する。
