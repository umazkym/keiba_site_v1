...
Threadsは`SNS_THREADS_IMAGE_MODE=public`（2026-09-25〜）で画像つきになり、画像つきの投稿にはリンクのカードが出ません。そこで、画像つきで公開できた投稿に、つかみの一言とそのレースのページへのリンクの返信を付け、返信の側にカード（題名にレース名）を出します（2026-09-26 利用者の選択と文の指定）。
- 重賞・メインレース：本文は`build_race_text`のまま。返信は「オールカマー（G2）の全頭の分析」＋レースのページ（`build_threads_race_reply`）。
