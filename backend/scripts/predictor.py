...
def calculate_and_save_horse_number_advantage(db: Session, start_date: date, end_date: date):
    """
    指定期間の全レース結果から、コース別の馬番有利不利指数を計算しDBに保存する。
    main.pyの`parse_race_result_horse_ai_common`のロジックを参考に実装。
    """
    print(f"Calculating horse number advantages from {start_date} to {end_date}...")
    ...
    if df.empty:
        print("No race results found in the specified period.")
        return
    ...
    if not ai_scores:
        print("Could not calculate any AI scores.")
        return
        
    df_with_ai = pd.concat(ai_scores)

    # ★★★ 修正箇所: `groupby`から`total_horses`を削除 ★★★
    # これにより、頭数が違っても同じコース条件であれば集計される
    advantage_groups = df_with_ai.groupby([
        'venue_name', 
        'course_type', 
        'distance', 
        # 'total_horses', # <- この行を削除またはコメントアウト
        'horse_number'
    ])['advantage_score'].mean().reset_index()

    advantages_to_save = advantage_groups.to_dict('records')
    
    database_loader.save_horse_number_advantages(db, advantages_to_save)
    print(f"Saved/Updated {len(advantages_to_save)} horse number advantage records.")# C:\Users\tnszk\program\GitHub\backend\scripts\predictor.py
from sqlalchemy.orm import Session
from sqlalchemy import func
from database import models
import pandas as pd
...
def calculate_and_save_matchups_for_race(db: Session, race_id: str, horse_ids: List[str]):
    if len(horse_ids) < 2: return
    past_results = db.query(models.Result, models.Race.venue_name, models.Race.race_date)\
        .join(models.Race, models.Result.race_id == models.Race.id)\
        .filter(models.Result.horse_id.in_(horse_ids)).all()
    races_grouped = defaultdict(list)
    for res, venue_name, race_date in past_results:
        races_grouped[res.race_id].append({
            'horse_id': res.horse_id, 'rank': res.rank,
    ...
    for past_race_id, participants in races_grouped.items():
        ...
        for i in range(len(participants)):
            for j in range(i + 1, len(participants)):
                ...
                if p1.get('rank') is not None and p2.get('rank') is not None:
                    key1, key2 = f"{p1['horse_id']}_vs_{p2['horse_id']}", f"{p2['horse_id']}_vs_{p1['horse_id']}"
                    history_entry = {'race_id': past_race_id, 'race_date': participants[0]['race_date'], 'venue_name': participants[0]['venue_name'], 'p1_horse_id': p1['horse_id'], 'p1_rank': p1['rank'], 'p2_horse_id': p2['horse_id'], 'p2_rank': p2['rank']}
                    if p1['rank'] < p2['rank']:
                        matchup_matrix[key1]['win'] += 1
                        matchup_matrix[key2]['loss'] += 1
                    elif p2['rank'] < p1['rank']:
    ...
    if matchup_matrix:
        ...
        if existing:
            ...
            db.add(models.Matchup(race_id=race_id, matchup_data=dict(matchup_matrix)))
        db.commit()

def calculate_and_save_horse_number_advantage(db: Session, start_date: date, end_date: date):
    print(f"Calculating horse number advantages from {start_date} to {end_date}...")
    results_query = db.query(
        models.Race.id, models.Race.venue_name, models.Race.course_type,
        models.Race.distance, models.Race.total_horses, models.Result.horse_number,
        models.Result.rank
    ).join(models.Result, models.Race.id == models.Result.race_id)\
    .filter(models.Race.race_date.between(start_date, end_date))\
    .filter(models.Result.rank.isnot(None))\
    ...
    df = pd.read_sql(results_query.statement, db.bind)
    if df.empty:
        print("No race results found in the specified period for advantage calculation.")
        return
    ai_scores = []
    for race_id, group in df.groupby('id'):
        n = group['total_horses'].iloc[0]
        if n is None or n < 2: continue
        e, sd = (n + 1) / 2.0, math.sqrt((n**2 - 1) / 12.0)
        if sd == 0: continue
        group['advantage_score'] = (e - group['rank']) / sd
        ai_scores.append(group)
    if not ai_scores:
