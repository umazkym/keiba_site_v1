...
* **2026-06-28**:
  * **トップ重賞欄の空振り防止と記事公開ゲート補正**:
    近日重賞APIが一時的に空配列や取得失敗になった場合でも、トップページとレースページの重賞欄が「確認中」表示だけで止まらないよう、2026年の中央重賞と主要地方交流重賞の日程フォールバックを `frontend/lib/grade-race-schedule.ts` に追加。API由来のレース詳細リンクを優先し、不足分だけ日程ページ・重賞ハブへの控えめな導線で補完する構成にした。サイトマップでは未確定レース番号の日程フォールバックを混ぜないよう `getWeeklyGradeRaces({ includeScheduleFallback: false })` を使う。記事生成パイプラインでは、公開直前バリデータが「無条件」を強すぎる表現として検出し、承認済み記事をロールバックしていたため、SEO CheckerとEditorの自動補正に「無条件」の置換を追加。`npx tsc --noEmit`、`npm run article:validate-links`、`npm run build` 成功。
* **2026-06-27**:
