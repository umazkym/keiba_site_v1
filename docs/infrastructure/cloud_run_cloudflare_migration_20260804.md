...
> **2026-09-26 の確認（利用者のダッシュボードの画面）**：動いているルールは「Extreme request burst protection」の1件で、式は `(http.request.method in {"GET" "POST"}) and not cf.client.bot`（すべてのパス）、IP ごとに10秒で100件、超えたら Block を10秒。検索ロボット（verified bot）は除外済みなので、検索の巡回には影響しない。画像・JS・CSS も数えるため、同じ回線で数ページを続けて開くと止まる。同日夜、利用者が式を上の「詳細ページだけ」（`/horses/`・`/trainers/` を除いた形）に変えた。件数と動作を変えたかは未確認。
