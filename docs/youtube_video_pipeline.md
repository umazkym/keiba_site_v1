...
- 会場別長尺: 開催会場ごとに1本。新馬戦・障害戦などAI偏差値の算出対象外レースを除き、対象レースを実際のレース番号順に収録します。各レース約6秒の1シーン内でAI偏差値上位3頭と全馬の位置取りを表示します。
- Short: 1日1本。G1、Jpn1、G2、Jpn2、G3、Jpn3、その他重賞の順で優先し、重賞がない日は11R、出走頭数、会場コードの順で決定します。
- 長尺タイトルは`M/D 会場｜全○R AI予想｜重賞名開催｜YYYY`の順とし、スマートフォン一覧で後半が省略されても日付、会場、全レース動画であることが先頭に残るようにします。タイトルには新馬戦などの除外情報を表示しません。重賞名は最上位の1件だけを「開催」と明記し、重賞1レースだけの動画に見える表現を避けます。
...
- GitHub Actionsは午後の翌日データ更新Workflowが成功した時点で連動起動します。18:20 JSTの予備cronも残し、単一concurrencyグループで直列化します。連動時は前段WorkflowのJST開始日の翌日、予備cronはJST翌日、手動実行は明示した対象日を正本とします。
- 翌日データが不完全な場合は10分間隔で最大3回確認します。2回目以降は同一プロセスの予測キャッシュを破棄し、IAP経由で最新DB状態を再取得します。

会場は1Rから最大レース番号までが連続し、すべての対象レースに実在馬名と有効なAI偏差値がある場合だけ公開対象になります。中央の「新馬」、地方競馬の「ゴールデンデビュー」「スパーキングデビュー」など主催者固有のデビュー戦名称、予測エンジンが対象外とする障害戦は動画から除外します。説明欄、動画メタデータ、Actions Summaryには除外したR番号・レース名・理由を残します。WorkflowではIAPトンネル経由のDBを正本とし、レース表に存在して予測表に存在しないレースも欠損として検知します。不完全な会場は会場単位で保留し、正常な会場は維持します。保留が発生したWorkflowは正常会場の処理後に失敗終了し、GitHubの通知とActions Summaryで把握できるようにします。
...
ワークフローはデータ更新連動と予備cronを単一concurrencyで直列化し、最大90分、Short 1本に固定します。アップロード前に動画本数、横長サムネイル数、状態確認回数からAPIクォータを概算し、設定上限を超える場合は投稿しません。日次実行ではMP4をartifactへ保存しません。手動`workflow_dispatch`かつ`dry_run=true`の場合だけ、横長19秒以内とShort 15秒の縮小レビューMP4を7日保存します。
...
レンダラーはプラットフォーム非依存の`VideoPackage`を返し、縦型カバー、個別レースpath、レース名、レース番号、標準版とTikTok専用版の動画pathを保持します。Threads、Instagram、Facebook、TikTok、Pinterest、Blueskyへの日次配信、UTM、投稿モード、外部設定は`docs/social_video_distribution.md`を参照します。Xの動画投稿は行いません。
