...
TARGET_EXTENSIONS = {
    ...
    '.md', '.mjs', '.yaml', '.yml', '.gitignore', '.env.example'
}
...
def is_target_file(file_path: str) -> bool:
    ...
    # 実際の環境設定は拡張子が.json等で終わっていても出力しない。
...
if __name__ == '__main__':
    # 起動場所にかかわらずリポジトリを読み、ローカル生成物へ出力する。
    repository_root = Path(__file__).resolve().parents[2]
    output_file = repository_root / '.local' / 'exports' / 'all_source_code.txt'
    output_file.parent.mkdir(parents=True, exist_ok=True)
    save_all_code_to_txt(str(repository_root), str(output_file))
