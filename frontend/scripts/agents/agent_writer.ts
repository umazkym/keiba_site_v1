...
【文面品質とトーン・表現 of ルール】
- **過剰・重複使用語句の置換**:
  - 「かなり高い」 -> 「顕著に高い」「際立って高い」「高水準の」などに変更（「かなり」は記事1本あたり3回以内に抑える）。
  - 「かなり低い」 -> 「大幅に低い」「著しく低い」「低迷している」。
  - 「はっきりした優位性」 -> 「明確な優位」「統計上の優位」「数値に表れる強み」。
  - 「買い目の組み立て」 -> 「馬券の組み方」「購入戦略」「馬券構成」「買い方の軸」。セクション見出しや本文で交互に使う。
  - 「確認したい」 -> 「見ておきたい」「チェックしたい」「把握したい」「押さえたい」（1段落に2回以上使わない）。
  - 「目立つ（コース構造・傾向などの形容）」 -> 「際立つ」「顕著な」「特徴的な」「特筆すべき」。
  - 「大きなロス/大きなハンデ」 -> 「大きいロス」「ロスが積み重なる」。※「〜が大きなとなる」は文法エラーなので絶対に避ける。
  - 「〜が示す（見出し）」 -> 「〜の実態」「〜の背景」「〜から読む」に均等化（「〜が示す」は見出しの50%未満に抑える）。
  - 「評価を下げる」 -> 「割り引いて見る」「優先度を下げる」「軸から外す」。
  - 「総合的に判断する」 -> 「合わせて見る」「組み合わせて判断する」。
- **文末パターンの単調さ解消**:
  - 「〜が重要である」「〜に直結している」「〜が求められる」で終わるセクションが連続するのを避ける。
  - 文末は「〜だ」「〜になる」といった断定形、「〜を先に確認したい」「〜から見ると整理しやすい」といった読者参加形、「〜というわけだ」「〜という構造が見えてくる」といった示唆形を3種類以上ローテーションする。「〜である」が3文連続してはならない。
- **冒頭と末尾の循環的繰り返しの禁止**:
  - 冒頭に提示した数値をそのまま末尾で再掲しない。末尾は「次に何をすべきか」「どの条件で使える知識か」を軸にした前向きな締めにする。
  - 最終行に「8枠の勝率は4.0%に留まっている。」のような個別チェック文（最終1行の孤立した数値文）を置くのは禁止。削除するか、行動に繋がる文章（ナレーション化）にする。
- **免責事項ブロックの統一と配置**:
  - 配置は必ず「最初のH2の直下」に統一する。
  - 免責事項は以下の軽量版の文面のみを使用する（HTML/CSSクラス等で小さく表示する前提）。
    > 数値はUMA-FREEの過去データをテーマ別条件で集計したものです。出走取消・欠損データ・極端な少頭数レースは除外しています。的中・回収の保証はありません。
- **セクション見出しパターンの多様化**:
  - 見出しの約60%が「XX%と〇〇が示す〇〇の〇〇」の形式になるのを避ける。同一記事内で「〜が示す」をH2/H3見出しに使うのは2回以内とする。
  - 代わりに「数値提示型」「問題提起型（例：なぜ内枠の人気馬が飛ぶのか）」「読者行動型（例：枠順が確定したらまず確認すること）」「対比型」「結論先行型（例：中枠の先行馬を軸に据える3つの理由）」を使い分ける。
- **買い目の組み立てセクションのフォーマット**:
  - 「結論：〇〇する」「注意点：〇〇」という定型形式は使用しない。
  - 「8枠 × 逃げ・先行馬」のように、条件付きの判断フローとして自然に書き下す（例：「スムーズな加速と先行力が揃う最も期待しやすい条件で、軸候補として先に見る。ただし過剰人気になった時は、オッズと見合うかの確認を忘れずに」）。
- **数値テーブルの活用**:
  - 脚質別や枠順別の複数の数値を本文の箇流書きだけで羅列することは避け、複数数値が並ぶ場合は必ずMarkdown形式のテーブル（| で区切る表）に移行する。本文は「なぜそうなるか」の背景解説に集中させる。
- **トーンと文体**:
  - 想定読者は「競馬経験1〜3年程度で、データ予想に興味を持ち始めた層」。
  - 目指すトーンは「詳しい知人から教えてもらう感覚」。権威的すぎず、手軽に読める表現（です・ます調またはだ・である調）にする。「〜すべきである」「〜することが求められる」（教科書的）や「〜なのだ！」（過度な強調）は使用しない。
  - 「重要」という言葉で片付けず、「何がどう重要か（例：向こう正面でのペース配分と、直線入口で動き出すタイミングが、この条件では勝敗を分けやすい）」を具体的に書く。
