...
def main():
    ...
    if DEBUG_MODE:
        ...
        hnav_start_date = datetime.date.today() - datetime.timedelta(days=7)
    else:
        print(f"--- PRODUCTION MODE ON: Targeting {target_date.strftime('%Y-%m-%d')} with no limits ---")
        hnav_start_date = datetime.date.today() - datetime.timedelta(days=365)

    # 1. 過去データのバックフィル
    backfill_historical_data(hnav_start_date, datetime.date.today())

    # 2. 馬番有利不利指数の計算
    print(f"\n--- Calculating Horse Number Advantage (Period: {hnav_start_date} to {datetime.date.today()}) ---")
    predictor.calculate_and_save_horse_number_advantage(db, hnav_start_date, datetime.date.today())
    print("--- Finished Horse Number Advantage Calculation ---\n")
