...
    // ▼▼▼▼▼ ここから修正 ▼▼▼▼▼
    // 「今日」の日付を管理するstateを追加し、初期値はnullにする
    const [todayStr, setTodayStr] = useState<string | null>(null);

    // このuseEffectはクライアントサイドでのみ実行される
    useEffect(() => {
        // サーバーとクライアントの初回レンダリングを一致させるため、
        // クライアントでのみ「今日」の日付を計算してstateにセットする
        const getClientTodayString = () => {
            const today = new Date(
                new Date().toLocaleString("en-US", { timeZone: "Asia/Tokyo" })
            );
            return today.toISOString().split("T")[0];
        };
        setTodayStr(getClientTodayString());
...
                    <Link
                        // ▼▼▼▼▼ ここから修正 ▼▼▼▼▼
                        href={`/races/${todayStr || new Date().toISOString().split("T")[0]}`}
                        // ▲▲▲▲▲ ここまで修正 ▲▲▲▲▲
...
                <div className="mb-4">
                    {/* ▼▼▼▼▼ ここから修正 ▼▼▼▼▼ */}
                    {/* `todayStr` がクライアントで設定されるまで何も表示しない */}
                    {todayStr && <SpecialPickCard date={todayStr} />}
                    {/* ▲▲▲▲▲ ここまで修正 ▲▲▲▲▲ */}
                </div>
                <RaceTabs
