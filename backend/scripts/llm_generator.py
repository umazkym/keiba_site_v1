...
def _get_per_race_instructions(race_id: str) -> str:
    ...
    conclusions = [
        ...
        "「堅い決着」か「波乱含み」かを最終診断し、それに応じた確認の方向性（上位を絞って見るか、広く見るか）を示す。",
        "穴馬を1頭挙げ、その馬が好走するための具体的な展開条件を1つ提示して終える。",
        "本命馬の最大のリスク要因を1つ指摘し、そのリスクが現実になった場合に浮上する馬に言及する。",
        "上位3頭を比較し、条件が最もそろっている1頭とその根拠を示して終える（「推奨」「おすすめ」とは書かない）。"
    ]
...
・「絶対」「必勝」「推奨」「おすすめ」「買うな」「儲かる」「爆益」などの断定・煽りは禁止。人気・枠・脚質の扱いを一段控えめに整理すること。
