...
- URL正規化時は、ページ構造のクエリと流入計測のクエリを区別する。2026-09-21監査で、レースURL等の転送がUTMを初回計測前に削除していたため、許可した計測キーを転送先へ保持する修正を追加した。canonicalはクエリなしの正規URLを維持し、修正のためにpage_viewを追加しない。参照元不明全体への寄与率は未確定で、反映後7完全日の同じ取得軸・分母で確認する。
- `measurement_quality.v1`は、GA4日別・流入別・入口別の合計差、欠損日、部分取得、分母を記録する。媒体ごとに取得範囲を照合し、未取得を0へ置換しない。GA4の収益帰属値とAdSenseの円建て収益は加算しない。
