...
def get_top_payout_hits(db: Session, days: int = 7, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    for race in races:
        ...
        if not race_returns: continue

        for r_return in race_returns:
            winning_numbers = {r_return.number_1}
            if r_return.number_2: winning_numbers.add(r_return.number_2)
            if r_return.number_3: winning_numbers.add(r_return.number_3)

            # 的中判定
            if winning_numbers.issubset(top_5_horse_numbers):
