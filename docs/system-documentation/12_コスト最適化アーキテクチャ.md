...
> **2026-08-04更新**: Vercel HobbyのCPU・ISR writes・Edge Requests超過により、フロントエンドはCloud Run + Cloudflareへ移行準備中です。本章のVercel $0前提や「月額$0」表現は過去構成の記録であり、今後は`docs/infrastructure/cloud_run_cloudflare_migration_20260804.md`の容量ゲートと実測請求を正とします。
...
| `docs/video/social_video_distribution.md:94` GCSライフサイクル | `gs://keiba-api-project-private-social-staging` に `Delete age:2` 設定済み。バケットは現在空 |
...
### 12.8 デプロイ後の実測（2026-08-27）

`keiba-site-v1` を再デプロイした直後の本番実測。

| 対象 | 修正前 | 修正後 |
|:---|---:|---:|
| `GET /api/v1/data/directories/course?limit=100&offset=0&indexable_only=false` | 10.4〜10.6秒 | **0.21秒**（3回連続で0.205〜0.215秒） |

二重集計の解消がそのまま効いており、約50分の1になった。
`p95_latency_ms` と `max_instance_saturated` は7日窓のため、
公開ゲートの判定は翌日以降の `Keiba Data Page Publication` で確認する。

#### フロントエンドのキャッシュに注意

`/about` は `Cache-Control: s-maxage=31536000`（1年）で Cloudflare にキャッシュされる。
デプロイ後も `cf-cache-status: HIT` のまま旧内容が返り続けるため、
**内容を変更したページは Cloudflare のキャッシュを個別にパージする必要がある**。
クエリ文字列を付けてオリジンへ抜けると新内容が確認できるので、
「デプロイが効いていない」と「キャッシュが古い」の切り分けはこれで行う。

```bash
curl -s "https://uma-free.com/about?cb=$(date +%s)" | grep -o "インフラ:[^\"<]*"
```

`.github/workflows/purge-race-cache.yml` は `race_date` 必須でレースページ専用のため、
静的ページのパージには使えない。Cloudflareダッシュボードの
Caching → Configuration → Purge Cache → Custom Purge でURL単位に消す。
