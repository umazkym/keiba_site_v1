...
・"grade_race_preview": 重賞レースのプレビュー記事。以下のルールに従う:
  ...
  - 結果回顧では、レース直後の読者だけでなく、同条件のレースを将来見返す読者にも価値が残るよう、以下のエバーグリーン要素を1つ以上含める:
    (a) 今回の結果で裏付けられた（または覆された）コース傾向の確認。枠番・脚質・馬場状態の影響が reference_data にある場合、次に同コースを見る際の確認材料として整理する。
    (b) 次回以降の同レースや同条件重賞を見る際に持ち越す判断軸。「このコースでは○○の条件が結果に直結した」という形で、具体的なチェック項目に落とす。
    ただし、入力にないデータで一般論を水増しすることは禁止。reference_data の範囲内で書けるものだけにする。
