        ...
        <div className="flex flex-col gap-12 py-12 px-4">
            <div className="flex flex-col gap-10">
                ...
                <div className="flex flex-wrap justify-center gap-4">
                    <Link href="/articles" className={`font-bold py-2 px-6 rounded-full transition-all duration-200 shadow-md ${!selectedCategory ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>{content.allCategoryButton}</Link>
                    {uniqueCategories.map((category) => (<Link href={`/articles?category=${encodeURIComponent(category)}`} key={category} className={`font-bold py-2 px-6 rounded-full transition-all duration-200 shadow-md ${selectedCategory === category ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>{category}</Link>))}
                </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
                {filteredArticles.map((article) => (<Link href={`/articles/${article.slug}`} key={article.slug} className="flex flex-col h-full border border-gray-200 rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 bg-white hover:translate-y-[-4px]"><div className="relative h-40 sm:h-44 md:h-48 w-full overflow-hidden"><Image src={article.eyecatch} alt={article.title} fill style={{ objectFit: 'cover' }} className="transition-transform duration-300 group-hover:scale-105" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" /></div><div className="flex flex-col gap-3 p-5"><span className="inline-block w-fit bg-blue-100 text-blue-800 text-xs font-semibold px-3 py-1 rounded-full">{article.category}</span><h2 className="text-xl font-bold text-gray-800 line-clamp-3">{article.title}</h2><p className="text-gray-600 text-sm mt-auto">{new Date(article.date).toLocaleDateString()}</p></div></Link>))}
            </div>
        </div>
    );
}
