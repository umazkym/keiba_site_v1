...
date: '2025-10-30'
description: >-
  ジャパンカップにおける東京芝2400mの直近データを分析し、日本馬の勝率93.3%という大きく優位性と、差し有利の定説を覆す先行馬の勝率40%の因果関係を整理。天皇賞・秋からのローテーション（複勝率42.1%）に基づく評価戦略を構築する。
...
複勝率42.1%を誇る天皇賞（秋）からの参戦組のうち、勝率40%の先行脚質を持つ馬を本命の軸に指定する。特に1〜4枠の好枠に入り込んだ場合は、迷わず厚め（購入点数の配分を増やす）の勝負に踏み切る。
