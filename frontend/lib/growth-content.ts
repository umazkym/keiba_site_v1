...
export const dataHubLinks: Array<LinkItem & { description: string }> = [
  {
    ...
    description: "良・稍重・重・不良による時計の変化と脚質の有利不利を、レース前に整理するためのページです。",
  },
  {
    label: "馬体重を見る",
    href: "/keiba-data/horse-weight",
    description: "パドック発表直後に慌てないための判断基準。成長分・疲労・ベスト体重への回帰を切り分けます。",
  },
  {
    label: "予想サイトを選ぶ基準",
    href: "/keiba-data/site-selection",
    description: "的中率だけでは分からない、無料範囲・指数の根拠・成績公開の透明度を比較するための視点です。",
  },
  {
    label: "AI予想の成績",
    href: "/results/accuracy",
    description: "AI偏差値の勝率・3着以内率を期間別・条件別に集計。得意条件と苦手条件の両方を掲載しています。",
  },
];
...
export const jockeyProfiles: JockeyProfile[] = [
  {
    ...
    lead:
      "ベテランならではのコース取りと仕掛け所の判断は健在で、特に京都・東京の芝中距離では折り合い重視の馬と好相性です。勝率だけでは見えにくい「乗り替わりで上積みが出る馬」を見極める目も持っています。",
    summary:
    ...
    checkpoints: [
      "前走で折り合いを欠いた馬に乗り替わる場合、距離短縮よりも騎手替わりの効果を優先して評価する。",
      "短距離の内枠で人気を集めている馬は、包まれて動けないリスクを考慮する。",
      "重賞では枠と脚質がハマった時に限り、頭固定まで引き上げる。",
    ],
    relatedArticles: [
  ...
  },
  {
    ...
    lead:
      "直線での進路選択と加速のタイミングに定評があり、東京芝の中距離では上位人気時の信頼度が高い騎手です。一方、短距離での外差し待ちは過剰人気になりやすく、展開次第で取りこぼすケースも目立ちます。",
    summary:
    ...
    checkpoints: [
      "1番人気でも、逃げ先行が多い短距離は差し遅れのリスクがある。",
      "乗り替わり＋距離延長の組み合わせでは、折り合いが改善するかがカギ。",
      "東京芝は枠番より、直線で進路を確保できるかどうかを重視したい。",
    ],
    relatedArticles: [
  ...
  },
  {
    ...
    checkpoints: [
      "差し一辺倒の馬では人気ほどの信頼は置きにくい。",
      "外枠先行が有利なコースなら、枠順発表後に評価を上乗せする余地あり。",
      "馬体重が大幅に減っている人気馬は、騎手評価だけでカバーしきれない。",
    ],
    relatedArticles: [
  ...
  },
  {
    ...
    checkpoints: [
      ...
      "外枠なら先行できるだけの二の脚があるかを出馬表で確認しておきたい。",
      "重馬場の日は、早めに動いた分が最後に残るかどうかが焦点になる。",
    ],
    relatedArticles: [
  ...
  },
  {
    ...
    lead:
      "好位に取り付いてから直線で進路を選ぶ騎乗スタイルで、無理に動かず馬の力を引き出すタイプです。東京・中山の中距離やダートの好位差しで成績が安定しています。",
    summary:
    ...
    checkpoints: [
      "外を回される差し馬は、直線だけで届く展開かどうかがカギ。",
      "短距離の内枠は包まれるリスクがあり、人気でも相手評価に留めたい。",
      "乗り替わりで先行可能な馬なら、位置取り改善をプラス評価できる。",
    ],
    relatedArticles: [
  ...
  },
  {
    ...
    checkpoints: [
      "同型の逃げ馬が揃うメンバー構成では、前半の消耗に注意。",
      "差し馬に騎乗する場合は、直線の進路より道中どこにつけるかが焦点。",
      "人気薄の先行馬は、当日の馬場が前残りかどうかとセットで判断する。",
    ],
    relatedArticles: [
  ...
  },
  {
    ...
    lead:
      "関西圏を主戦場に、先行して粘り込む形で堅実な成績を残しています。ダート1400m〜1800mでの好位差しや、中京芝2000mのように持続力が問われるコースで強みが出やすい騎手です。",
    summary:
    ...
    checkpoints: [
      "逃げ馬が多いレースでは、無理に競り合わない形が取れるかが焦点。",
      "外枠の先行馬は1コーナーまでの距離ロスを考慮する。",
      "重賞で相手関係が上がった時に、同じ位置を取れるかがカギになる。",
    ],
    relatedArticles: [
  ...
  },
  {
    ...
    lead:
      "関東圏の平場やダート短距離で、オッズ以上の走りを見せるケースがある騎手です。人気を集めるタイプではありませんが、「人気薄で何か一頭拾いたい」ときに注目したい存在です。",
    summary:
      "中山ダート1200m、東京ダート1600mが主な活躍の場。前に行ける馬や外枠からスムーズに運べる馬では相手候補に残しやすくなります。",
    strengths: ["中山ダート1200m", "東京ダート1600m", "関東平場", "人気薄の好位差し"],
    checkpoints: [
      "上位人気の差し馬では、展開待ちになりすぎないか注意。",
      "短距離はスタートと枠順が結果を左右するため、先に確認する。",
      "人気薄を拾う場合は、馬場と脚質が噛み合う時に限定する。",
    ],
    relatedArticles: [
  ...
  },
  {
    ...
    lead:
      "馬のリズムを大事にしながら運ぶタイプで、特に差し馬で人気が控えめな時にオッズ以上の走りを見せることがあります。福島や新潟などローカル開催でも注目したい騎手です。",
    summary:
      "東京・中山の差し馬、福島や新潟のローカル開催が狙い目。馬場が外差しに寄る日は特に要チェックです。内で詰まりやすい馬は割り引きます。",
    strengths: ["東京芝マイル", "福島芝1800m", "新潟外回り", "人気薄の差し馬"],
    checkpoints: [
      "内枠の差し馬は進路確保が難しく、枠順との相性を見る。",
      "直線勝負になりすぎる馬は、ペースが流れるかどうかがカギ。",
      "ローカルでは馬場の内外差を当日のレース傾向から判断する。",
    ],
    relatedArticles: [
  ...
  },
  {
    ...
    lead:
      "京都・阪神を中心に、好位を取ってしぶとく脚を使うタイプ。芝のマイルから中距離、ダート1800mあたりの安定感が強みです。後ろからになると展開待ちの色が出やすい点には注意が必要です。",
    summary:
      "京都・阪神の芝マイル〜中距離とダート1800mが得意ゾーン。人気馬の安定騎乗に定評がありますが、外を回すだけの差し馬では展開依存が強まります。",
    strengths: ["京都芝1600m", "阪神芝1600m", "関西ダート1800m", "好位差し"],
    checkpoints: [
      "人気馬でも後ろからになる場合は、展開が向くかの判断が必要。",
      "先行馬では同型の数と枠順の組み合わせで評価が変わる。",
      "馬場が重くなった日は、瞬発力型より持続力型の馬との相性が良くなる。",
    ],
    relatedArticles: [
  ...
  },
  {
    ...
    checkpoints: [
      "開催後半で内が荒れている時は、枠順だけで判断しない。",
      "短距離はスタートの安定感が結果を左右する。",
      "重賞で相手が強くなると、平場での成績がそのまま通用しないことがある。",
    ],
    relatedArticles: [
  ...
  },
  {
    ...
    checkpoints: [
      "洋芝実績がない馬は、人気でも馬場適性を先に確認したい。",
      "外枠で距離ロスが大きくなりそうなレースでは割り引く。",
      "人気薄は馬場と位置取りが合う時だけ拾うのが無難。",
    ],
    relatedArticles: [
  ...
  },
  {
    ...
    lead:
      "馬のリズムを大事にしながら運ぶスタイルで、逃げ・先行馬の粘り込みや長距離戦での折り合いが光る騎手です。中山芝2500mや札幌芝2000mのように、長丁場で折り合えるかが問われる舞台が得意です。",
    summary:
      "中山芝2500m、札幌芝2000m、逃げ先行馬、距離延長で折り合える馬が得意ゾーン。展開が速くなりすぎる短距離では慎重に見たいところです。",
    strengths: ["中山芝2500m", "札幌芝2000m", "逃げ・先行馬", "長距離の折り合い"],
    checkpoints: [
      "単騎で行けるか、同型が多いかをメンバー構成から先に確認する。",
      "人気馬でも展開が速くなりすぎると取りこぼすリスクあり。",
      "長距離では前走の折り合いと距離延長の負荷をセットで評価する。",
    ],
    relatedArticles: [
