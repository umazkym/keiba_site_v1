...
def create_predictions_for_race(db: Session, race_id: str) -> Optional[List[Dict[str, Any]]]:
    ...
    if "新馬" in target_race.race_name or "障害" in target_race.race_name or target_race.course_type == '障':
        is_unpredictable = True
    
    all_horse_scores = []
    for horse in shutuba_horses:
        # 予測対象外レースの場合、パフォーマンススコアの計算をスキップ
        ...
        all_horse_scores.append({
            'horse_id': horse.horse_id, 'horse_name': horse.name, 'horse_number': horse.horse_number,
    ...
    # 予測対象外、または有効なスコアが2頭未満の場合は、偏差値計算を行わない
    if is_unpredictable or len(valid_scores) < 2:
        df['deviation_score'] = None # nullにする
        df['mark'] = "—"
    else:
        ...
        if std == 0:
            ...
            df['deviation_score'] = df['raw_score'].apply(lambda x: 50.0 + 10 * (x - mean) / std if pd.notna(x) else None) # nanではなくNone
