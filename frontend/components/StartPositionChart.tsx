...
const HorseMarker = ({ horse, position }: { horse: HorsePrediction, position: number }) => (
    <div  
        ...
        style={{  
            // ★修正: 上下にはみ出さないよう、10pxのオフセットを追加
            top: `${10 + (horse.horse_number - 1) * 15}px`,
            left: `${position}%`,
...
export const StartPositionChart = ({ predictions }: { predictions: HorsePrediction[] }) => {
    ...
    return (
        <div className="my-2 p-4 bg-white border rounded-lg shadow-inner">
            {/* ★修正: コンテナの高さをマーカーの配置に合わせて再計算 */}
            <div className="relative w-full bg-green-100 rounded" style={{ height: `${(sortedByNumber.length - 1) * 15 + 20}px` }}>
