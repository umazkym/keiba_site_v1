...
export const RaceAnalysisComment: React.FC<RaceAnalysisCommentProps> = ({
    ...
    const generateAnalysisComment = (): string => {
        ...
        if (courseType === 'turf' && distance === 2000) {
            ...
            const texts = [
                `ダート短距離戦はスタートダッシュが命。逃げ・先行馬が圧倒的に有利なコース形態です。AIが予測した先行力上位の馬を中心に馬券を組み立てるのがセオリーと言えます。`,
                `電撃のダート1200mでは、テンの速さがそのまま着順に直結しやすい傾向にあります。スタート直後のポジション争いを制する逃げ馬候補に最も注目が集まります。`,
                `スピード勝負となるダート短距離。過去の統計からも、最初のコーナーを好位で回れる馬の勝率が際立っています。AI推奨の先行馬の出走有無には要チェックです。`
            ];
            comment = getRandomText(texts, seed);
            ...
            const texts = [
                ...
                `AI偏差値データが拮抗しており、絶対的な本命が不在の混戦ムード。騎手の仕掛けのタイミング一つで順位が入れ替わる、波乱含みのレースと言えそうです。`,
                `上位勢の能力差が少なく、展開次第でどの馬にもチャンスがある条件。AIは複数頭の能力を高く評価しており、オッズ妙味のある穴馬の発掘に期待がかかります。`
            ];
            comment = getRandomText(texts, seed);
        }
        ...
        if (horseCount <= 8) {
            const texts = [
                `\n出走頭数が${horseCount}頭と比較的少頭数のため、AIが高い確度で予想した上位馬へ人気と結果が集中しやすくなる傾向があります。`,
                `\n${horseCount}立ての少頭数戦。紛れが少なく実力通りの決着になりやすいため、AI偏差値上位の信頼度が増すレースと言えます。`,
                `\n少頭数（${horseCount}頭）の構成となります。コースロスが起きにくいため、馬本来の能力値（AIスコア）を素直に信頼しやすい条件です。`
            ];
